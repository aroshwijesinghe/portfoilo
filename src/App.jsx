...
/* ══════════════════════════════════════════════
   PORTFOLIO CONFIG — Edit your details here
   ══════════════════════════════════════════════ */
const PROFILE = {
...
/* ═══════════════════════════════════════
   OFFICIAL SVG ICONS — Real brand logos
   ═══════════════════════════════════════ */
const LangIcons = {
  python: (
...
function useEmojiCursor() {
  const set = useCallback((emoji) => {
    if (!emoji) { document.body.style.cursor = "default"; return; }
  ...
  const reset = useCallback(() => { document.body.style.cursor = "default"; }, []);
  return { set, reset };
}
...
function handleEmailClick(email, e) {
  ...
  setTimeout(() => {
    navigator.clipboard.writeText(email).then(() => {
      alert(`Email copied to clipboard: ${email}\n\nOpen your email client and paste it.`);
    }).catch(() => {
      alert(`Email: ${email}`);
    });
  }, 500);
...
function AnimBg({ dark }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 65; i++) pts.push({ x: Math.random() * cv.width, y: Math.random() * cv.height, vx: (Math.random() - .5) * .3, vy: (Math.random() - .5) * .3, r: Math.random() * 2 + .8, o: Math.random() * .4 + .1 });
    const col = dark ? "0,212,255" : "0,120,200";
    const mul = dark ? 1 : .55;
    const loop = () => {
      ctx.clearRect(0, 0, cv.width, cv.height);
      pts.forEach(p => { p.x += p.vx; p.y += p.vy; if (p.x < 0 || p.x > cv.width) p.vx *= -1; if (p.y < 0 || p.y > cv.height) p.vy *= -1; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fillStyle = `rgba(${col},${p.o * mul})`; ctx.fill(); });
      for (let i = 0; i < pts.length; i++) for (let j = i + 1; j < pts.length; j++) { const dx = pts[i].x - pts[j].x, dy = pts[i].y - pts[j].y, d = Math.sqrt(dx * dx + dy * dy); if (d < 130) { ctx.beginPath(); ctx.moveTo(pts[i].x, pts[i].y); ctx.lineTo(pts[j].x, pts[j].y); ctx.strokeStyle = `rgba(${col},${.06 * (1 - d / 130) * mul})`; ctx.lineWidth = .5; ctx.stroke(); } }
...
export default function Portfolio() {
  ...
  const t = isDark ? {
    ...
    glass: "rgba(255,255,255,.03)", gBorder: "rgba(255,255,255,.06)",
    glassS: "rgba(255,255,255,.05)", gBorderS: "rgba(255,255,255,.08)",
    inputBg: "rgba(255,255,255,.04)", inputB: "rgba(255,255,255,.08)",
    ...
    glass: "rgba(255,255,255,.7)", gBorder: "rgba(0,0,0,.08)",
    glassS: "rgba(255,255,255,.85)", gBorderS: "rgba(0,0,0,.1)",
    inputBg: "rgba(0,0,0,.03)", inputB: "rgba(0,0,0,.1)",
  ...
  return (
    <>
      ...
      {/* Nextjs icon inner color as CSS variable */}
            ...
            <div className="hero-grid" style={{ display: "flex", alignItems: "center", gap: 60 }}>
              <div style={{ flex: 1 }}>
                ...
                <div className="srow s5" style={{ display: "flex", gap: 16, marginTop: 40 }}>
                  {[{ icon: I.gh, href: PROFILE.github, l: "GitHub" }, { icon: I.li, href: PROFILE.linkedin, l: "LinkedIn" }, { icon: I.mail, href: PROFILE.email, isEmail: true, l: "Email" }, { icon: I.phone, href: `tel:${PROFILE.phone}`, l: "Phone" }].map(s => (
                    <a key={s.l} href={s.isEmail ? "#" : s.href} onClick={(e) => { s.isEmail ? handleEmailClick(s.href, e) : (e.preventDefault(), window.open(s.href, '_blank')); }} rel="noopener noreferrer" title={s.l}
                      style={{ width: 42, height: 42, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: t.glass, border: `1px solid ${t.gBorderS}`, color: t.muted, transition: "all .3s", textDecoration: "none", cursor: "pointer" }}
                ...
                <div className="float-a" style={{ position: "relative" }}>
                  <div className="hero-img" style={{ width: 300, height: 300, borderRadius: "50%", background: `linear-gradient(135deg,${t.accent}40,${t.accent2}40)`, padding: 3 }}>
                    <div style={{ width: "100%", height: "100%", borderRadius: "50%", overflow: "hidden", background: t.bgAlt, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <img src="./profile.jpg" alt={PROFILE.name} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%" }}
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "100px 24px" }}>
            ...
            <div className="agrid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "start" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                {[{ l: "University", v: "UoM", e: "🎓" }, { l: "Year", v: "2nd Year", e: "📅" }, { l: "Focus", v: "ML / AI", e: "🤖" }, { l: "Projects", v: `${PROJECTS.length}+`, e: "🚀" }].map(s => (
                  <div key={s.l} style={{ background: t.glass, border: `1px solid ${t.gBorder}`, backdropFilter: "blur(20px)", borderRadius: 16, padding: 24, textAlign: "center" }}>
        ...
        <Sec id="skills">
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "100px 24px" }}>
            ...
            <div className="sgrid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 24 }}>
              {SKILLS.map(g => (
                <div key={g.category} className="glow-border" style={{ background: t.glass, border: `1px solid ${t.gBorder}`, backdropFilter: "blur(20px)", borderRadius: 20, padding: 28, position: "relative" }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                    {g.items.map(sk => (
                      <span key={sk.name} className="stag" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 16px", borderRadius: 50, fontSize: ".88rem", background: t.inputBg, border: `1px solid ${t.inputB}`, color: t.muted, cursor: "default" }}>
                        {LangIcons[sk.icon] || <span>•</span>} {sk.name}
                      </span>
                    ))}
                  </div>
                </div>
        ...
        <Sec id="projects">
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "100px 24px" }}>
            ...
            <div className="pgrid" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 24 }}>
              {PROJECTS.map((p, i) => (
                <div key={i} className="pcard glow-border" style={{ background: t.glass, border: `1px solid ${t.gBorder}`, backdropFilter: "blur(20px)", borderRadius: 20, overflow: "hidden", position: "relative" }}>
                  ...
                  <div style={{ padding: 32 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                      ...
                      {p.link && <a href={p.link} target="_blank" rel="noopener noreferrer" style={{ color: t.faint, transition: "color .3s", display: "flex", alignItems: "center", gap: 6, fontSize: ".82rem", textDecoration: "none", fontFamily: "'JetBrains Mono',monospace" }}
                        onMouseEnter={e => e.currentTarget.style.color = p.color} onMouseLeave={e => e.currentTarget.style.color = t.faint}>
                        GitHub {I.arr}
                      </a>}
                    </div>
            ...
            <div style={{ maxWidth: 700, margin: "0 auto", position: "relative" }}>
              ...
              {TIMELINE.map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 32, marginBottom: 40, position: "relative" }}>
                  ...
                  <div style={{ background: t.glass, border: `1px solid ${t.gBorder}`, backdropFilter: "blur(20px)", borderRadius: 16, padding: 24, flex: 1 }}>
            ...
            <div className="cgrid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48 }}>
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                  {[{ icon: I.mail, l: "Email", v: PROFILE.email, h: PROFILE.email, isEmail: true }, { icon: I.phone, l: "Phone", v: PROFILE.phone, h: `tel:${PROFILE.phone}` }, { icon: I.gh, l: "GitHub", v: "aroshwijesinghe", h: PROFILE.github }, { icon: I.li, l: "LinkedIn", v: "Arosh Wijesinghe", h: PROFILE.linkedin }].map(c => (
                    <a key={c.l} href={c.isEmail ? "#" : c.h} onClick={(e) => { c.isEmail ? handleEmailClick(c.h, e) : (e.preventDefault(), window.open(c.h, '_blank')); }} rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 16, textDecoration: "none", color: t.muted, transition: "color .3s", cursor: "pointer" }}
              ...
              <div style={{ background: t.glass, border: `1px solid ${t.gBorder}`, backdropFilter: "blur(20px)", borderRadius: 20, padding: 32 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <button className="btn-p" style={{ alignSelf: "flex-start" }} onClick={() => alert("Thank you! I'll get back to you soon.")}>{I.send} Send Message</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <footer style={{ position: "relative", zIndex: 1, borderTop: `1px solid ${t.gBorder}`, padding: "32px 24px" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
            ...
            <div style={{ display: "flex", gap: 20 }}>
              {[{ i: I.gh, h: PROFILE.github }, { i: I.li, h: PROFILE.linkedin }, { i: I.mail, h: PROFILE.email, isEmail: true }].map((s, j) => (
                <a key={j} href={s.isEmail ? "#" : s.h} onClick={(e) => { s.isEmail ? handleEmailClick(s.h, e) : (e.preventDefault(), window.open(s.h, '_blank')); }} rel="noopener noreferrer" style={{ color: t.faint, transition: "color .3s", cursor: "pointer" }}
